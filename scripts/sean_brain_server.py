...
def generate_sean_brain_response(user_msg):
    ...
    elif any(w in msg_lower for w in ["달력", "캘린더", "ui", "ux", "디자인", "우선순위", "3d", "레이어", "도서관", "미학", "finom"]):
        return (
            "형! 내 뇌의 **[박시현 대표 독보적 UI/UX 감각 & 3D 캘린더 매트릭스]** 신경망이야!\n\n"
            "1. 🏛️ **대도서관 디지털화 철학** : 정보 나열 금지! 지금 필요한 카드만 솟아오르고, 확인 완료 시 서가에 책 꽂듯 제자리에 안착.\n"
            "2. 🚀 **Zero Friction Onboarding** : 화면 들어오자마자 1초 만에 알 수 있는 `Today Quest Beam` + `[Space]` 원키 클리어링 루프.\n"
            "3. 🔥 **3단계 우선순위 시각 계층화** :\n"
            "   - **1순위 (🔥 중요 마일스톤)** : 자본금 납입, 스튜디오 입주, HRB 등기 ➔ 네온 레드 & 18px 최상단 돌출\n"
            "   - **2순위 (⚡ AI 구독)** : Suno, Cursor, Gemini, ElevenLabs ➔ 네온 시안 & 10px 돌출\n"
            "   - **3순위 (💤 자동이체)** : 월세, 전기세, 방송수신료, 보험료 ➔ 딥 슬레이트 스텔스 톤 (시각 소음 0%)\n"
            "4. 🌌 **휠 스크롤 3D 레이어 시프팅** : 1레이어는 상단 메뉴 0% 순수 달력, 휠 아래로 굴리면 달력이 3D 뒤로 가라앉으며 2레이어 컨트롤 덱이 떠오름!"
        )
    else:
        return (
            ...
            "5. 🏛️ **[Executive UI/UX 클러스터]** : 대도서관 메타포 & 3D 캘린더 매트릭스\n\n"
